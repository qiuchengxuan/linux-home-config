...
(setq robot-mode-keywords '(
    ...
    ("\\[[^\]]+\\]+" . font-lock-constant-face)
    ;;builtin
    ("^\\(Documentation\\|Library\\|Variables\\)" . font-lock-builtin-face)
    ("\\(FOR\\|IN RANGE\\)"                       . font-lock-builtin-face)
    ;;function-name
    ("[A-Z][a-z0-9]+\\( [A-Z][a-z0-9]+\\)+" . font-lock-function-name-face)
    ("[A-Z][a-z0-9]+   "                    . font-lock-function-name-face)
    ;;Variables
